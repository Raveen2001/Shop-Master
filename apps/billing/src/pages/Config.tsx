...
const ConfigPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      // Validate URLs
      const urlFields: (keyof AppConfig)[] = [
        "apiBaseUrl",
        "imageBaseUrl",
        "printerApiBaseUrl",
      ];
    ...
    <Box className="mx-auto max-w-2xl p-6">
      ...
      <Card>
        <CardContent className="space-y-6">
          ...
          <TextField
            label="Printer API Base URL"
            value={config.printerApiBaseUrl}
            onChange={(e) =>
              handleInputChange("printerApiBaseUrl", e.target.value)
            }
            fullWidth
            placeholder="http://localhost:9001"
            helperText="API endpoint for printer operations (optional)"
          />
